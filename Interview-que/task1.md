...
Create a minimal HTTP server using only Node’s built-in http module (no Express/Koa/fastify or third-party libs). The server must expose routes that: <br> 
(1) send a welcome message, and <br> 
(2) return an array of objects as JSON. <br>
...
```
	...
	•	Response: 200 OK, header Content-Type: application/json, 
	•   body example:
